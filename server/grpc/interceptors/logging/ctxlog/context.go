...
var (
	...
	nullLogger   = log.NewHelper(log.DefaultLogger)
)
...
func Extract(ctx context.Context) *log.Helper {
	l, ok := ctx.Value(ctxMarkerKey).(*ctxLogger)
	if !ok || l == nil {
		return nullLogger
	}
...
func ToContext(ctx context.Context, logger *log.Helper) context.Context {
	l := &ctxLogger{
		logger: logger,
	}
